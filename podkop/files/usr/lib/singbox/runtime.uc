...
const SB_MANAGED_SERVICE_MARKER = getenv("SB_MANAGED_SERVICE_MARKER") || "Podkop Plus managed sing-box service for binary variants";
...
function install_managed_service_script() {
    let tmp = "/etc/init.d/sing-box.podkop-plus." + owner_pid();
...
function service_listen_address_value(settings) {
    ...
    log_message("Failed to determine the listening IP address. Please open an issue to report this problem: https://github.com/ushan0v/podkop-plus/issues", "error");
    return "";
}
