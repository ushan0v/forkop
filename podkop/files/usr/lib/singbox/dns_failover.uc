...
const STATE_FILE = getenv("PODKOP_DNS_FAILOVER_STATE_FILE") || RUNTIME_STATE_DIR + "/dns-failover.json";
const PID_FILE = getenv("PODKOP_DNS_FAILOVER_PID_FILE") || RUNTIME_STATE_DIR + "/dns-failover.pid";
const DNS_FAILOVER_UC = getenv("PODKOP_DNS_FAILOVER_UC") || LIB_DIR + "/singbox/dns_failover.uc";
const SERVICE_BIN = getenv("PODKOP_BIN") || "/usr/bin/podkop-plus";
...
function log_message(message, level) {
    command_success_from_args([ "logger", "-t", "podkop-plus", "[" + as_string(level || "info") + "] DNS failover: " + as_string(message) ]);
}
