...
const PODKOP_VERSION = getenv("PODKOP_VERSION") || constants.PODKOP_VERSION || "";
const PODKOP_CONFIG = getenv("PODKOP_CONFIG") || constants.PODKOP_CONFIG || "/etc/config/" + CONFIG_NAME;
const PODKOP_SERVICE_NAME = getenv("PODKOP_SERVICE_NAME") || constants.PODKOP_SERVICE_NAME || "podkop-plus";
const PODKOP_RELEASE_REPO = getenv("PODKOP_RELEASE_REPO") || constants.PODKOP_RELEASE_REPO || "ushan0v/podkop-plus";
const PODKOP_LUCI_VIEW_DIR = getenv("PODKOP_LUCI_VIEW_DIR") || constants.PODKOP_LUCI_VIEW_DIR || "/www/luci-static/resources/view/podkop_plus";
...
const SYSTEM_INFO_CACHE_TTL = int(getenv("PODKOP_SYSTEM_INFO_CACHE_TTL") || "3600");
...
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || constants.RT_TABLE_NAME || "podkop";
...
const NFT_COMMON_SET_NAME = getenv("NFT_COMMON_SET_NAME") || constants.NFT_COMMON_SET_NAME || "podkop_plus_subnets";
const NFT_PORT_SET_NAME = getenv("NFT_PORT_SET_NAME") || constants.NFT_PORT_SET_NAME || "podkop_plus_ports";
const NFT_IP_PORT_SET_NAME = getenv("NFT_IP_PORT_SET_NAME") || constants.NFT_IP_PORT_SET_NAME || "podkop_plus_ip_ports";
const NFT_INTERFACE_SET_NAME = getenv("NFT_INTERFACE_SET_NAME") || constants.NFT_INTERFACE_SET_NAME || "podkop_plus_interfaces";
...
function check_logs() {
    ...
    let rendered = status_capture([ "podkop-logs" ], command_output_from_args([ "logread" ]));
    if (rendered.output != "")
...
function podkop_logs_fixture() {
    let rendered = status_capture([ "podkop-logs" ], read_stdin());
    if (rendered.output != "")
        print(rendered.output);
...
function show_config(visibility) {
    visibility = as_string(visibility || "masked");
    if (!file_exists(PODKOP_CONFIG)) {
        nolog("Configuration file not found");
        return 1;
    }
    if (visibility == "raw")
        print(as_string(fs.readfile(PODKOP_CONFIG)));
    else
        print(status_output([ "podkop-config-masked", PODKOP_CONFIG ], null));
    return 0;
}

function show_version() {
    print(PODKOP_VERSION, "\n");
    return 0;
}
...
function get_luci_app_version() {
    let path = PODKOP_LUCI_VIEW_DIR + "/main.js";
    ...
    for (let line in split(as_string(data), "\n")) {
        ...
        if (matched != null && matched[1] == "PODKOP_LUCI_APP_VERSION")
...
function system_info_cache_is_valid() {
    ...
    if (now > 0 && generated_at > 0 && SYSTEM_INFO_CACHE_TTL > 0 && now - generated_at >= SYSTEM_INFO_CACHE_TTL)
        return false;
    return cache.podkop_version == PODKOP_VERSION && cache.luci_app_version == get_luci_app_version();
}
...
function build_system_info() {
    let podkop_latest_version = first_line_value("/tmp/podkop-plus.latest-version.cache", "unknown");
    ...
    return {
        ...
        podkop_latest_version: podkop_latest_version || "unknown",
        luci_app_version,
        sing_box_version,
...
function neutralize_zapret_defaults() {
    log_message("Standalone zapret is not neutralized automatically; Podkop Plus uses /opt/zapret/nfq/nfqws as an external provider and manages only its own NFQUEUE range.", "info");
    return 0;
}
...
function dnsmasq_has_podkop_dns() {
    return module_success(DNS_APPLY_UC, [ "has-podkop-dns" ]);
}
...
else if (mode == "podkop-logs-fixture")
    exit(podkop_logs_fixture());
