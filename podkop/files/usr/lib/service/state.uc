...
const DEFAULT_PENDING_RELOAD_FILE = getenv("PODKOP_PENDING_RELOAD_FILE") || "/var/run/podkop-plus/reload.pending";
const DEFAULT_SERVICE_INIT = getenv("PODKOP_SERVICE_INIT") || "/etc/init.d/podkop-plus";
...
function run_pending_reload_if_requested(path, init_script) {
    ...
    command_success_from_args([ "logger", "-t", "podkop-plus", "[info] Applying pending Podkop Plus reload" ]);
...
function reload_sing_box_runtime() {
    command_success_from_args([ "logger", "-t", "podkop-plus", "[info] Reloading sing-box runtime" ]);
    if (command_success_from_args([ "/etc/init.d/sing-box", "reload" ]))
        return;

    command_success_from_args([ "logger", "-t", "podkop-plus", "[fatal] Failed to reload sing-box. Aborted." ]);
    exit(1);
}
...
function podkop_runtime_network_configured(rt_table, nft_table, mark) {
...
function podkop_running(rt_table, nft_table, mark) {
    return sing_box_service_running() && podkop_runtime_network_configured(rt_table, nft_table, mark);
}

function podkop_stably_running(rt_table, nft_table, mark, min_age) {
    return sing_box_service_stable(min_age) && podkop_runtime_network_configured(rt_table, nft_table, mark);
}

function wait_podkop_stable_start(rt_table, nft_table, mark, min_age, timeout) {
    timeout = int(timeout || 8);
    while (timeout > 0) {
        if (podkop_stably_running(rt_table, nft_table, mark, min_age))
            return true;
...
function dnsmasq_signature_body(settings, dnsmasq, legacy_dnsmasq_present) {
    ...
    body = signature_add_value(body, "dhcp.@dnsmasq[0].podkop_server", option(dnsmasq, "podkop_server", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].podkop_noresolv", option(dnsmasq, "podkop_noresolv", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].podkop_cachesize", option(dnsmasq, "podkop_cachesize", ""));
    body = signature_add_value(body, "dhcp.podkop_plus.present", arg_bool(legacy_dnsmasq_present) ? "1" : "0");
...
function uci_dnsmasq() {
    return {
        ...
        podkop_server: uci_get("dhcp.@dnsmasq[0].podkop_server"),
        podkop_noresolv: uci_get("dhcp.@dnsmasq[0].podkop_noresolv"),
        podkop_cachesize: uci_get("dhcp.@dnsmasq[0].podkop_cachesize")
    };
}
...
if (mode == "file-first-line")
    ...
    exit(podkop_stably_running(ARGV[1], ARGV[2], ARGV[3], ARGV[4]) ? 0 : 1);
else if (mode == "wait-podkop-stable-start")
    exit(wait_podkop_stable_start(ARGV[1], ARGV[2], ARGV[3], ARGV[4], ARGV[5]) ? 0 : 1);
    ...
    exit(print_signature_hash(dnsmasq_signature_body(uci_settings(), uci_dnsmasq(), uci_exists("dhcp.podkop_plus"))) ? 0 : 1);
